...
class PoseEditContracts(unittest.TestCase):
    def test_reference_order_is_pose_then_background_and_character(self):
        ...
        self.assertIsNone(UC.draw._prepare_pose_edit_images({}, "illustrious", (64, 64)))
    ...
    def test_klein_reference_pipeline_appends_two_images_to_both_conditionings(self):
        ...
        def call(names, methods, **kwargs):
            ...
            if name == "ReferenceLatent":
                ...
                return (result,)
            self.fail(name)
        # The reference steps run through the declarative pipeline runner.
        with patch.object(UC.pipeline, "_call_node_method", call), \
             patch.object(UC.models.flux_klein, "_conditioning_debug", return_value={}), \
             patch.object(UC.models.flux_klein, "_latent_debug", return_value={}):
    ...
    def test_normal_node_output_composites_pose_as_separate_alpha_layer(self):
        ...
        out = UC.render._render_unicanvas_state_to_rgba(json.dumps(state))
        self.assertEqual(out.getpixel((0, 0)), (0, 40, 80, 255))
        self.assertEqual(out.getpixel((1, 0)), (255, 0, 0, 255))
        state["layers"][0]["visible"] = False
        self.assertEqual(UC.render._render_unicanvas_state_to_rgba(json.dumps(state)).getpixel((1, 0)), (0, 40, 80, 255))
