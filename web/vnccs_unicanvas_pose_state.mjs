...
/**
 * The normal pass of a layer when it is current (Plan 08.3 relight): camera-space normals packed
 * as RGB = n * 0.5 + 0.5 over `meta.rect` (world pixels). A pose layer's pass must match its
 * mannequins and camera; other layers only carry one when it was set for them.
 */
export function currentNormalPass(layer) {
    const meta = layer?.poseNormalMeta, canvas = layer?.poseNormalCanvas;
    if (!canvas || !meta?.rect) return null;
    if (layer.type === "pose" && meta.key !== poseIdKey(layer.pose)) return null;
    return { canvas, meta };
}

/** State-cache form of the normal pass (a PNG of the rect-sized normal canvas plus its key). */
export function serializePoseNormal(layer) {
    const meta = layer?.poseNormalMeta, canvas = layer?.poseNormalCanvas;
    if (!canvas || !meta?.rect) return null;
    return { key: meta.key ?? null, rect: { ...meta.rect }, dataURL: canvas.toDataURL("image/png") };
}

export async function restorePoseNormal(layer, stored, loadImage) {
    if (!stored?.dataURL || !stored.rect) return;
    try {
        const image = await loadImage(stored.dataURL);
        const canvas = globalThis.document.createElement("canvas");
        canvas.width = Math.max(1, image.naturalWidth || image.width);
        ...
        layer.poseNormalMeta = { key: stored.key == null ? null : String(stored.key), rect: { ...stored.rect } };
    } catch (_) { /* A missing normal pass is regenerated by the next commit. */ }
}
