...
The Pose Library ships a read-only **Interactions** category (repository `vnccs_interaction_presets`, files in `pose_presets/Interactions/`): hug, handshake, high five, kiss on the cheek, arm around the shoulder, princess carry, piggyback, punch and block, sitting side by side, whisper, pointing at each other, dancing pair and a three-person group photo. They are v3 `scene_pose` assets whose characters carry poses and placement but no body, name or color. Loading any multi-character pose scene outside animation mode now replaces the whole scene through `loadCharacterSceneLibraryAsset`; `inheritSlotIdentity` (`web/vnccs_pose_interactions.mjs`) keeps the body, name and color of the mannequin already in each slot when the asset has none, and UniCanvas keeps the bound references by slot as before.

Presets are authored at the default proportions and store `interaction.reference_heights` (per slot). After the bodies load, `applyInteractionHeightScaling` scales each character's offset from the pair's midpoint, and its height above the ground, by its height ratio to that reference (a carried character follows its `supports` slot), so an adult and a child still meet horizontally. Heights come from the vertical extent of each morphed mesh; the bone-length proportion sliders are not included, and vertical offsets (a child's hand is lower) are left to the user, since Pose Studio has no effector API to snap the passive mannequins' hands.

The presets are generated by `node scripts/interaction_presets/build.mjs` from `scripts/interaction_presets/presets.mjs`, which poses a headless copy of Pose Studio's rig (`rig.mjs`, the same MakeHuman pack and bone hierarchy) with two-bone IK towards the contact points; `tests/test_pose_interaction_presets.mjs` checks that the committed files match the generator and that every listed contact meets at the default proportions.
