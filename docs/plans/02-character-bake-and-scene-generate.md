...
3. Mannequins **without** a character reference are never baked. They stay in the scene pass
   composite as mannequins, and the scene model replaces them from the positive prompt and
   whatever reference images it gets (for example the edit-model reference uploads,
   `Picture 2..5`). This is exactly what an edit model does with a mannequin today.
4. GENERATE itself keeps its current behavior. The scene pass **always** runs, with whatever
   Prompt the user wrote, empty included. The bakes are only an extra step before it.
...
5. **Model:** the current engine when it is QiE2511 or Klein9b. Otherwise it is the **Bake
   model**, set in the **UniCanvas settings popover** (the gear, `openUniCanvasSettings` in
   `web/vnccs_unicanvas.js`, the same panel that holds the Remove background backend rows). It
   is a new "Character bake" group: family QiE2511 / Klein9b, the preset card, and optional
   steps/cfg overrides. The default is the first *ready* preset of those families. This removes
   today's "Pose layers require QiE2511 or Klein9b ... or hide the pose layer" block for the
   scene pass, because only bakes need those families.
...
4. **Scene pass (always):** runs the normal `draw()` path with the current engine, exactly as
   GENERATE does today, over the bbox composite. Baked characters show their baked pixels, and
   mannequins without a reference stay as mannequin pixels for the model to replace from the
   prompt and reference images. The Prompt field is used as it is today, including when it is
   empty. No special case, no skip.
5. With no bake candidates, GENERATE is exactly today's GENERATE. Bake-only is available
   from the card rows and the layer context menu (manual bakes), not from GENERATE.
...
**What changes versus today's single-pose branch:** today GENERATE with a visible pose layer in
the bbox either blocks (no character reference: `poseCharacterIssue`) or sends one `pose_edit`
request instead of a scene pass. After this plan, a missing reference no longer blocks (that
mannequin goes to the scene pass as-is), and a bound reference means bake first, then scene
pass. The `pose_edit` request shape stays the same. It is only issued per character by the
bake.
...
- `web/vnccs_unicanvas.js`: the `draw()` pre-pass and scene pass rules, `historyGroup`,
  the bake-count badge on GENERATE, the settings popover "Character bake" group, and serialization of
  the two canvases.
...
- `bake.spec.mjs`, with `page.route` stubbing `/vnccs/unicanvas/draw` (a fixture PNG of a
  ...
  - GENERATE with one bound and one unbound mannequin -> exactly one bake request (the bound
    one), then one scene request whose image still contains the unbound mannequin's pixels.
    One undo reverts everything.
  - Empty prompt -> the scene request is still sent, with the empty prompt, as today.
  - No pose layers or nothing to bake -> the request is identical to today's GENERATE.
